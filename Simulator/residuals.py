...
def simulator_residuals():
    for degree in range(1, 6):
        vsf_func = lambda B0_env: numerical_simulator(
            B0_env=B0_env.unsqueeze(0).unsqueeze(0), interpolator = polynomial_interpolator(degree)
        )
        residuals, elapsed = residuals_of_interpolation(vsf_func)
        df = pd.DataFrame(data=residuals, columns=[f"Residuals"])
        df.to_csv(f"./Charts/data/Degree_{degree}_Residuals.csv")
...
def analytic_residuals():
    ...
    vsf_func = lambda B0_env: sinc_3d(B0_env.unsqueeze(0).unsqueeze(0), *differentiate_3d(B0_env.unsqueeze(0).unsqueeze(0)), t)[:, :, 1, 1, 1]
    residuals, elapsed = residuals_of_interpolation(vsf_func)
    df = pd.DataFrame(data=residuals, columns=[f"Residuals"])
    df.to_csv(f"./Charts/data/Analytic_Residuals.csv")
